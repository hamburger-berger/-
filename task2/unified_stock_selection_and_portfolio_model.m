...
% =========================================================================
% 四股票组合投资完整建模代码（统一主模型版）
%
% 与《四股票组合投资完整建模方案——统一主模型版》保持一致：
%
% 一、前端选股模块
% 1. 读取 BaoStock 多股票日行情数据；
% 2. 截取最近 3 个完整自然年样本；
% 3. 使用前复权收盘价（adjustflag = 2）；
% 4. 剔除样本区间内停牌天数超过 30 天的股票；
% 5. 对齐交易日、删除缺失交易日；
% 6. 计算对数收益率；
% 7. 计算相关距离 d_ij = sqrt(2(1-rho_ij))；
% 8. 使用 average-linkage 层次聚类；
% 9. 通过轮廓系数 + CH 指标联合确定最优类别数 k*；
% 10. 按规则选出最终 4 只股票；
% 11. 对最终 4 股进行相关性复核，要求任意两只股票相关系数 <= 0.7。
%
% 二、统一组合优化主模型
% 目标函数：
%       min_w  w' * Sigma * w - lambda * (mu' * w - rf)
%
% 约束：
%       sum(w_i) = 1
%       0.05 <= w_i <= 0.40
%       w_i + w_j <= 0.65
%       在“目标收益约束最小风险组合”中额外要求 mu' * w >= R0
%
% 输出四类组合：
% 1. 等权重组合；
% 2. 最小风险组合；
% 3. 目标收益约束最小风险组合；
% 4. 最优切点组合（lambda 扫描后夏普比率最大）。
%
% 默认输入：
%       all_50_stocks_2019_2025_baostock.csv
%
% 默认输出目录：
%       unified_portfolio_matlab_outputs
%
% 所需工具箱：
